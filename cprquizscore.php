...
$qs = mysqli_fetch_assoc(mysqli_query($conn,
    "SELECT quiz_label, max_mistakes FROM quiz_settings WHERE quiz_id = 'cpr'"));
$quizLabel   = $qs ? htmlspecialchars($qs['quiz_label']) : 'CPR Training - Quiz';
$maxMistakes = $qs ? (int)$qs['max_mistakes'] : 3;

$stmt = mysqli_prepare($conn,
    "SELECT * FROM quiz_scores WHERE user_id=? AND quiz_name='cpr'
...
<div class="main">
  <h2 class="title">QUIZ SCORE FOR<br>
    <?php echo strtoupper(str_replace([' - Quiz', ' - QUIZ'], '', $quizLabel)); ?>
  </h2>
  <p><b>STEPS ACHIEVED:</b> <?php echo $score['steps'] . "/" . $score['total_steps']; ?></p>
  <p><b>PERCENTAGE:</b> <?php echo $score['percentage']; ?>%</p>
  <p><b>MISTAKES:</b> <?php echo $score['mistakes']; ?>/<?php echo $maxMistakes; ?></p>
  <p><b>OVERALL SCORE:</b> <?php echo $score['status']; ?></p>
</div>
