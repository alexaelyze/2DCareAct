...
// ── Fetch simulation settings for dynamic title and max_mistakes ──
$ss = mysqli_fetch_assoc(mysqli_query($conn,
    "SELECT sim_title, max_mistakes FROM simulation_settings WHERE sim_id = 'mobilitysimulation'"));
$simTitle    = $ss ? htmlspecialchars($ss['sim_title']) : 'Assisted Mobility Simulation';
$maxMistakes = $ss ? (int)$ss['max_mistakes'] : 3;

// ── Fetch this user's score ──
$stmt = mysqli_prepare($conn,
    "SELECT * FROM simulation_scores WHERE user_id=? AND simulation_name='mobilitysimulation'
...
<head>
<meta charset="UTF-8">
<title>Assisted Mobility Simulation Score</title>
<style>
:root { --primary: #536dfe; }
...
<div class="main">
  ...
  <h2 class="title">SIMULATION SCORE FOR<br>
    <?php echo strtoupper($simTitle); ?>
  </h2>
