...
$qs = mysqli_fetch_assoc(mysqli_query($conn,
    "SELECT quiz_label, max_mistakes FROM quiz_settings WHERE quiz_id = 'vitals'"));
$quizLabel   = $qs ? htmlspecialchars($qs['quiz_label']) : 'Checking Vital Signs - Quiz';
$maxMistakes = $qs ? (int)$qs['max_mistakes'] : 3;

// ── Fetch this user's score ──
$stmt = mysqli_prepare($conn,
    "SELECT * FROM quiz_scores WHERE user_id=? AND quiz_name='vitals'
...
<head>
<meta charset="UTF-8">
<title>Vital Signs Quiz Score</title>
<style>
:root{--primary:#536dfe}
...
.title{color:var(--primary);font-size:32px;font-weight:bold;margin-bottom:35px;line-height:1.3;text-align:center;}
p{font-size:18px;margin:18px 0;text-align:center;}
p1{font-size:18px;margin:18px 0;text-align:left;} 
