...
$qs = mysqli_fetch_assoc(mysqli_query($conn,
    "SELECT quiz_label, max_mistakes FROM quiz_settings WHERE quiz_id = 'hygiene'"));
$quizLabel   = $qs ? htmlspecialchars($qs['quiz_label']) : 'Elderly Hygiene and Grooming - Quiz';
$maxMistakes = $qs ? (int)$qs['max_mistakes'] : 3;

// ── Fetch this user's score ──
$stmt = mysqli_prepare($conn,
    "SELECT * FROM quiz_scores WHERE user_id=? AND quiz_name='hygiene'
...
<head>
<meta charset="UTF-8">
<title>Elderly Hygiene Quiz Score</title>
<style>
:root { --primary: #536dfe; }
