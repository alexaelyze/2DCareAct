 ...
 * ---------------------------------------------------------------
 */
define('MODULE_ID',   'module3');          // ← change per module
define('MODULE_LINK', 'module3.php');      // ← change per module
define('MODULE_NUM',  '3');                // ← change per module (for <title>)
...
if ($isInstructor && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['new_image'])) {
    ...
    if ($_FILES['new_image']['error'] === UPLOAD_ERR_OK) {
        ...
        if ($ext !== 'png') {
            // PNG validation — redirect with error flag
            header("Location: " . $moduleFile . "?edit=1&err=notpng");
            exit();
        }
        $folder = 'm' . MODULE_NUM . '/';
        if (!is_dir($folder)) { mkdir($folder, 0755, true); }
        $filename = 'step_' . time() . '.png';
...
<!-- PNG ERROR TOAST -->
<?php if ($pngError): ?>
<div class="toast-error" id="pngToast">⚠️ Only PNG files are supported. Please upload a .png image.</div>
<script>
...
<?php if ($isInstructor && $editMode): ?>
  ...
  <form method="POST" id="descForm">
    ...
    <input type="hidden" name="update_desc" value=""><!-- ensures key exists -->
  </form>
        ...
        <div>
          <label>
            <div class="upload-slot" id="uploadSlot"
              ...
              <div class="upload-hint">Click to upload PNG image</div>
            </div>
            <input type="file" name="new_image" id="newImg" accept=".png"
                   style="display:none"
...
function previewImg(input) {
  ...
  if (!file.name.toLowerCase().endsWith('.png')) {
    alert('⚠️ Only PNG files are supported. Please select a .png image.');
    input.value = '';
    return;
  }
