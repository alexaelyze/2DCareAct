 ...
 * ---------------------------------------------------------------
 */
define('MODULE_ID',   'module1');          // ← change per module
define('MODULE_LINK', 'module1.php');      // ← change per module
define('MODULE_NUM',  '1');                // ← change per module (for <title>)
...
if ($isInstructor && isset($_GET['delete_step'])) {
    ...
    $stmt = mysqli_prepare(
        $conn,
        "DELETE FROM module_contents WHERE id=? AND module_id=?"
    );

    // FIX: store constant in a variable first
    $mid = MODULE_ID;

    // Bind parameters (id = int, module_id = string)
    mysqli_stmt_bind_param($stmt, "is", $sid, $mid);

    // Execute
    mysqli_stmt_execute($stmt);

    // Redirect back to edit mode
    header("Location: " . MODULE_LINK . "?edit=1");
    exit();
}
...
function saveOrder() {
  ...
  .then(() => location.reload()); // rebuild layout properly
}
