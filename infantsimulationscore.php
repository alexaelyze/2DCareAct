...
$ss = mysqli_fetch_assoc(mysqli_query($conn,
    "SELECT sim_title, max_mistakes FROM simulation_settings WHERE sim_id = 'infantsimulation'"));
$simTitle    = $ss ? htmlspecialchars($ss['sim_title']) : 'Infant Care Simulation';
$maxMistakes = $ss ? (int)$ss['max_mistakes'] : 3;

// ── Fetch this user's score ──
$stmt = mysqli_prepare($conn,
    "SELECT * FROM simulation_scores WHERE user_id=? AND simulation_name='infantsimulation'
...
<head>
<meta charset="UTF-8">
<title>Infant Care Simulation Score</title>
<style>
:root { --primary: #536dfe; }
