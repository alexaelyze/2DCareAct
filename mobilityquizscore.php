...
$qs = mysqli_fetch_assoc(mysqli_query($conn,
    "SELECT quiz_label, max_mistakes FROM quiz_settings WHERE quiz_id = 'mobility'"));
$quizLabel   = $qs ? htmlspecialchars($qs['quiz_label']) : 'Assisted Mobility - Quiz';
$maxMistakes = $qs ? (int)$qs['max_mistakes'] : 3;

// ── Fetch this user's score ──
$stmt = mysqli_prepare($conn,
    "SELECT * FROM quiz_scores WHERE user_id=? AND quiz_name='mobility'
...
<head>
<meta charset="UTF-8">
<title>Assisted Mobility Quiz Score</title>
<style>
:root { --primary: #536dfe; }
